		...
		<div id="chatter_header" style="background-color:{{ $discussion->color }}">
			<div class="container">
				...
				<h1>{{ $discussion->title }}</h1><span
						class="chatter_head_details">Geplaatst In {{ Config::get('chatter.titles.category') }}<a
							class="chatter_cat"
			...
			<div class="chatter-alert alert alert-danger">
				<div class="container">
					<p><strong><i class="chatter-alert-danger"></i> {{ Config::get('chatter.alert_messages.danger') }}
						</strong> Los de volgende fouten op:</p>
					<ul>
						...
						@else
							<div class="col-md-3 left-column">
								...
								<div class="chatter_sidebar">
									<button class="btn btn-primary" id="new_discussion_btn"><i class="chatter-new"></i>
										Nieuwe {{ Config::get('chatter.titles.discussion') }}</button>
									<a href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i>
										Alle {{ Config::get('chatter.titles.discussions') }}</a>
							...
							<div class="col-md-9 right-column">
		                		...
		                		<span class="chatter_posts">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id || $isAdmin))

										<div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
		                					<i class="chatter-warning"></i>Weet je zeker dat je dit antwoord wilt verwijderen?
		                					<button class="btn btn-sm btn-danger pull-right delete_response">Ja</button>
		                					<button class="btn btn-sm btn-default pull-right">Nee</button>
		                				</div>
										<div class="chatter_post_actions">
			                				<p class="chatter_delete_btn">
			                					<i class="chatter-delete"></i> Verwijder
			                				</p>
			                				<p class="chatter_edit_btn">
			                					<i class="chatter-edit"></i> Bewerk
			                				</p>
			                			</div>
									@endif
		        					...
		        					{{ strtoupper(substr(Auth::user()->email, 0, 1)) }}
											...
											<form id="chatter_form_editor"
												...
												<div id="editor">
													@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
														<label id="tinymce_placeholder">Voer hier je antwoord in...</label>
													...
													@elseif($chatter_editor == 'trumbowyg')
														<textarea class="trumbowyg" name="body"
																  placeholder="Voer hier je antwoord in...">{{ old('body') }}</textarea>
													@endif
												</div>
										...
										<div id="discussion_response_email">
											<button id="submit_response" class="btn btn-success pull-right"><i
														class="chatter-new"></i> Verstuur Antwoord
											</button>
											@if(Config::get('chatter.email.enabled'))
												<div id="notify_email">
													...
													<span>Stuur mij een bericht als iemand reageerd</span>
													<label class="switch">
								...
								@else
									...
									<div id="login_or_register">
										<p>Log in of registreer om een reactie te plaatsen</p>
									</div>
			...
			@if(Config::get('chatter.sidebar_in_discussion_view'))
				<div id="new_discussion_in_discussion_view">
					...
					<form id="chatter_form_editor_in_discussion_view"
						...
						<div class="row">
							<div class="col-md-7">
								...
								<input type="text" class="form-control" id="title" name="title"
									   placeholder="Titel voor {{ Config::get('chatter.titles.discussion') }}"
						...
						<div id="editor">
							@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
								<label id="tinymce_placeholder">Voeg hier de content voor de discussie toe</label>
						...
						<div id="new_discussion_footer">
							<input type='text' id="color" name="color"/><span class="select_color_text">Selecteer een kleur (optional)</span>
							<button id="submit_discussion" class="btn btn-success pull-right"><i
										class="chatter-new"></i> Aanmaken {{ Config::get('chatter.titles.discussion') }}
							</button>
							<a href="/{{ Config::get('chatter.routes.home') }}" class="btn btn-default pull-right"
							   id="cancel_discussion">Annuleer</a>
			...
			<script>
                $('document').ready(function () {
                    ...
                    $('.chatter_edit_btn').click(function () {
                        ...
                        container.append('<div class="chatter_update_actions"><button class="btn btn-success pull-right update_chatter_edit"  data-id="' + id + '" data-markdown="' + markdown + '"><i class="chatter-check"></i> Update Antwoord</button><button href="/" class="btn btn-default pull-right cancel_chatter_edit" data-id="' + id + '"  data-markdown="' + markdown + '">Annuleer</button></div>');
