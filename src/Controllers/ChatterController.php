...
class ChatterController extends Controller
{
    ...
    {
        if (!Auth::check()) {
            return \Redirect::to('/'.config('chatter.routes.login').'?redirect='.config('chatter.routes.home'))->with('flash_message', 'Maak een account aan voordat je iets plaatst.');
        }
    }
    ...
    {
        if (!Auth::check()) {
            return \Redirect::to('/'.config('chatter.routes.register').'?redirect='.config('chatter.routes.home'))->with('flash_message', 'Maak eerst een account aan.');
        }
    }
}
